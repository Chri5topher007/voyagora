...
function Home() {
  ...
  useEffect(() => {
    fetch('import.meta.env.VITE_API_URL/tours').then(res => res.json()).then(data => { setAllTours(data); setFilteredTours(data); });
    fetch('import.meta.env.VITE_API_URL/events').then(res => res.json()).then(data => setEvents(data));
    fetch('import.meta.env.VITE_API_URL/community').then(res => res.json()).then(data => setGems(data));
  }, []);
