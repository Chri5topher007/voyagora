...
export default function TravelerDashboard() {
  ...
  const fetchData = async () => {
    ...
    const bRes = await fetch('import.meta.env.VITE_API_URL/bookings/mine', { headers: { 'Authorization': 'Bearer ' + token } });
    setBookings(await bRes.json());
    
    const wRes = await fetch('import.meta.env.VITE_API_URL/wishlist/mine', { headers: { 'Authorization': 'Bearer ' + token } });
    ...
    <div className="min-h-screen bg-slate-50 flex">
      <aside className="w-64 bg-slate-900 text-white p-6 hidden md:flex flex-col">
        ...
        <button 
          ...
          className="w-full bg-red-600 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-red-700 mb-4 transition shadow-lg"
        >
          🆘 Emergency SOS
        </button>

        <button onClick={logout} className="text-left text-red-400 hover:text-red-300 flex items-center gap-2">🚪 Logout</button>
      </aside>
