...
export class TourService {
  ...
  async getAllTours(query: { search?: string; maxPrice?: string }) {
    ...
    return tours.map((t: any) => { const avgRating = t.reviews.length > 0 ? t.reviews.reduce((acc: number, r: any) => acc + r.rating, 0) / t.reviews.length : 0; return { ...t, avgRating: avgRating.toFixed(1), reviewCount: t.reviews.length }; });
  }
  ...
  async getTourById(id: string) {
    ...
    const avgRating = tour.reviews.length > 0 ? tour.reviews.reduce((acc: number, r: any) => acc + r.rating, 0) / tour.reviews.length : 0;
  ...
  async getOrganizerStats(organizerId: string) {
    ...
    const tourIds = tours.map((t: any) => t.id); const eventIds = events.map((e: any) => e.id);
    const bookings = await this.prisma.booking.findMany({ where: { OR: [{ tourId: { in: tourIds } }, { eventId: { in: eventIds } }] } });
    const totalRevenue = bookings.reduce((acc: number, b: any) => acc + b.organizerPayout, 0);
    const chartData = [];
    for (let i = 6; i >= 0; i--) { const d = new Date(); d.setDate(d.getDate() - i); const dateString = d.toISOString().split('T')[0]; const dayBookings = bookings.filter((b: any) => new Date(b.createdAt).toISOString().split('T')[0] === dateString); chartData.push({ date: d.toLocaleDateString('en-US', { weekday: 'short' }), revenue: dayBookings.reduce((acc: number, b: any) => acc + b.organizerPayout, 0), bookings: dayBookings.length }); }
