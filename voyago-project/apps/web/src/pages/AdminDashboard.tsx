...
import { authFetch, isLoggedIn, logout as apiLogout } from '../lib/api';
...
export default function AdminDashboard() {
  ...
  const fetchPending = async () => {
    ...
    const res = await authFetch(`/community/pending`);
  ...
  const approve = async (id: string) => {
    await authFetch(`/community/` + id + '/approve', { method: 'PATCH' });
    fetchPending();
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      <aside className="w-64 bg-slate-900 text-white p-6 hidden md:flex flex-col">
        ...
        <button onClick={async () => { await apiLogout(); navigate('/'); }} className="text-left text-red-400 hover:text-red-300 flex items-center gap-2 mt-auto">🚪 Logout</button>
      </aside>
