...
import { API_URL, authFetch, logout as apiLogout, isLoggedIn } from '../lib/api';
...
export default function TravelerDashboard() {
  ...
  const fetchData = async () => {
    ...
    if (!isLoggedIn() || role !== 'TRAVELER') return navigate('/');

    const bRes = await authFetch(`/bookings/mine`);
    setBookings(await bRes.json());

    const wRes = await authFetch(`/wishlist/mine`);
  ...
  const logout = async () => { await apiLogout(); navigate('/'); };
